  ...
  <div class="w-full flex flex-col gap-1">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 text-xs sm:text-sm gap-1">
      <div v-for="(day, index) in days" :key="index"
        class="h-30 border border-base-content/10 p-2 flex flex-col gap-1 bg-base-100" :class="{
        ...
        <div v-for="event in getEventsForDay(day).slice(0, 3)" :key="event.id"
          class="text-xs p-2 py-0.5 truncate text-white" :class="colorClass(event.color)">
          {{ event.title }}
        </div>
