...
# ============================
# Function remove private task
# ============================
async def remove_private_task(
    task_id: str, 
    user_id: str
) -> dict:
    
    # Raise if user_id not provided
    if not user_id:
        raise HTTPException(status_code=400, detail="User ID not provided")

    # Raise if task_id not provided
    if not task_id:
        raise HTTPException(status_code=400, detail="Task ID not provided")
