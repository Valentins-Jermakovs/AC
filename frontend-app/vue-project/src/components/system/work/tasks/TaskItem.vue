<template>
    <div class="w-full h-16 flex items-center p-2 border border-base-300 bg-base-200 hover:bg-base-300 hover:cursor-pointer"
        @click="selectTask">
        <div class="flex flex-col flex-1 px-4">
            <h1 class="font-medium text-md">{{ shortTitle }}</h1>
            <p class="text-xs text-base-content/60">{{ shortDescription }}</p>
        </div>
        <div class="pr-4 text-xs text-base-content/60">
            Due {{ task.dueDate }}
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        shortTitle() {
            ...
            return this.task.title.length > 20
                ? this.task.title.slice(0, 20) + '...'
                : this.task.title
        },
