    ...
    <div class="card bg-base-200 border border-base-300 rounded-none p-4">
      ...
      >
        <font-awesome-icon
          ...
          class="text-3xl text-base-content/40 animate-pulse"
        />
...
export default {
  ...
  methods: {
    ...
    renderBarChart() {
      const labels = this.visitsStore.visits.daily_breakdown.map((d) => d.date)

      const data = this.visitsStore.visits.daily_breakdown.map((d) =>
        d.visits > 0 ? this.formatMinutes(d.seconds / d.visits) : 0,
      )
            ...
            tooltip: {
              ...
              callbacks: {
                label: (ctx) => {
                  ...
                  if (!day.visits) return 'No visits'

                  const avgSeconds = Math.round(day.seconds / day.visits)
    ...
    renderLineChart() {
      ...
      const data = this.visitsStore.visits.daily_breakdown.map((d) => this.formatMinutes(d.seconds))
