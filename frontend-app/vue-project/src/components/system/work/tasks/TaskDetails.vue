<template>
  <div class="border border-base-300 p-4 bg-base-100 flex flex-col gap-5 h-full overflow-y-auto">
    ...
    <div class="flex flex-col gap-5">
      <h1 class="font-semibold text-2xl flex items-center gap-3">
        <!-- Ikona -->
        <font-awesome-icon icon="fa-solid fa-clipboard-list" class="text-primary shrink-0" />

        <!-- Title -->
        <span class="wrap-break-word whitespace-pre-wrap min-w-0 flex-1">
          {{ task.title }}
        </span>
      </h1>
...
export default {
  ...
  computed: {
    ...
    descriptionRemainingCharsUpdate() {
      return this.descriptionMaxLength - (this.editForm.description?.length || 0)
    },
    error() {
